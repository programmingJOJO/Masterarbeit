  ...
  $.fn.tinyNav = function (options) {
    ...
    return this.each(function () {
      ...
      if ($nav.is('ul,ol')) {
        ...
        if (!settings.header) {
          var activeElement = $(l_namespace_i).find('li').index($(l_namespace_i).find('li a.active').parent());
          $select
            .find(':eq(' + activeElement + ')')
            .attr('selected', true);
        }
...
var Helpers = {
	...
	wrapCharacters: function(string) {
        var chars = jQuery.map(string.split(''), function(c) {
            return '<span>' + c + '</span>';
        });
...
var App = {
	...
	init: function() {
		//App.initFAQPage();
		App.initArchiveTitles();
		App.initTwitter();
		App.initEventBindings();
		App.formatSingleEvent();
    App.initStagenames();

		App.initSearchResults();

    App.initMobileNav();

    App.initTrailerOverlay();
	},

  initStagenames: function() {
    jQuery(document).ajaxComplete(function() {
      jQuery('.label-room').each(function() {
        jQuery(this).html(function() {
          return jQuery(this).html().replace('STG-', '<strong>STG</strong>-');
        });
      });
      jQuery('.field-name-field-session-room .field-item').each(function() {
        jQuery(this).html(function() {
          return jQuery(this).html().replace('STG-', '<strong>STG</strong>-');
        });
      });
      jQuery('.views-field-field-session-room .field-content').each(function() {
        jQuery(this).html(function() {
          return jQuery(this).html().replace('STG-', '<strong>STG</strong>-');
        });
      });
  ...
  initTrailerOverlay: function() {
    jQuery('.header-trailer').click(function(e) {
      e.preventDefault();
      var mq = window.matchMedia("(min-width:768px)");
      if(mq.matches && window.console) {

        jQuery('<div>')
          .attr('id', 'trailer-overlay')
          .prependTo(jQuery('body'));

        jQuery('<div>')
          .attr('id', 'overlay-close')
          .click(function() {
            jQuery('#trailer-overlay').remove();
          })
          .appendTo(jQuery('#trailer-overlay'));

        jQuery('<div>')
          .attr('id', 'overlay-video')
          .html('<iframe width="640" height="360" src="https://www.youtube.com/embed/I2LdeozUozg?rel=0&amp;showinfo=0&amp;autoplay=1" frameborder="0" allowfullscreen></iframe>')
          .appendTo(jQuery('#trailer-overlay'));
      }
    });
  },
  ...
  initMobileNav: function() {
    jQuery('#block-system-main-menu > .menu').tinyNav();
  },

	initFAQPage: function() {

		if(jQuery('.page-event-faq').length > 0 ) {
			jQuery('<div>').addClass('toc').html('<ol class="toc_list"></ol>').prependTo('#content');
			
			var counter = 1;
			jQuery('.views-accordion-ncms_faqs-page_all_accordion-header').each(function() { 
				jQuery(this).attr('id', 'faq-' + counter);
				counter = counter + 1;
				jQuery('<li>').html('<a href="#' + jQuery(this).attr('id') + '">' + jQuery(this).html() + '</a>').appendTo('.toc_list');
			});
		}
	},

	initArchiveTitles: function() {
		/*jQuery('.page-archive .panels-flexible-row').each(function() { 
			var title = jQuery(this).find('.view-ncms-archive-sessions .views-row-1 .views-field-title-1 .field-content').html();
			jQuery('<h2>').html(title).prependTo(this); 
		});*/
	},

	initTwitter: function() {
		jQuery('#menu-bar .tweets-pulled-listing').slick({
			infinite: true,
			speed: 500,
			autoplaySpeed: 5000,
			fade: true,
	...
	initSearchResults: function() {
		jQuery('#page .search-results li').each(function() { 
			var datetime = jQuery(this).find('.search-info');
			jQuery(datetime).prependTo(jQuery(this));
		});
	},

	closeOverlayVideo: function() {
		jQuery('#vid-overlay').remove();
		if(jQuery('#spVideoContainer').length > 0 ) {
			jQuery('#spVideoContainer').show();
		}
	},

	overlayVideo: function(link, width, height, url) {
		if(link !== false) {
			...
			jQuery('<iframe>')
				.attr({
					width: 640,
					height: 320,
					src: '//www.youtube.com/embed/' + link + '?autoplay=1&rel=0',
					frameborder: 0,
					allowfullscreen: 1
				})
				.prependTo('#vid-overlay');
	...
	formatSingleEvent: function() {
		if(jQuery('body.node-type-session').length > 0) {
			jQuery('#page-title').detach().prependTo('.node-content');
			//jQuery('.field-name-field-session-track-ref').detach().prependTo('.node-content');
		}
	},

	initEventBindings: function() {
		/*jQuery('a[data-video]').on('click', function(e) {
			e.preventDefault();
			App.overlayVideo(jQuery(this).attr('data-video'));
		});

		jQuery('a.vvlink.video-overlay').on('click', function(e) {
			e.preventDefault();
			jQuery('#spVideoContainer').hide();
			App.overlayVideo(false, 640, 361, jQuery(this).attr('href'));
		});
		jQuery*/
	}
...
jQuery(document).ready(function($) {
	...
	jQuery('#block-views-ncms-archive-sessions-block-all,#block-views-ncms-archive-speakers-block-all,.view-ncms-testimonials .view-grouping').addClass('clearfix');


	/* Background rotation */

	var bgimages = [
		'rp001.jpg','rp002.jpg','rp003.jpeg','rp004.jpg','rp005.jpeg','rp006.jpg','rp007.jpg','rp008.jpg','rp009.jpg','rp010.jpg','rp011.jpg','rp012.jpg','rp013.jpg','rp014.jpg','rp015.jpg','rp016.jpg','rp017.jpg',
		'rp019.jpg','rp020.jpeg','rp021.jpg','rp022.jpg','rp023.jpg','rp024.jpg','rp025.jpg','rp026.jpg',
		'rp027.jpg','rp028.jpg','rp029.jpg','rp030.jpg','rp031.jpg','rp032.jpg','rp033.jpg','rp034.jpg','rp035.jpg','rp036.jpg','rp037.jpg','rp038.jpg','rp039.jpg','rp040.jpg','rp041.jpg','rp042.jpg','rp043.jpg',
		'rp044.jpg','rp045.jpg','rp046.jpg','rp047.jpg','rp048.jpg','rp049.jpg','rp050.jpg','rp051.jpg'
	];

    var setBG = function() {
    	$('#block-block-50').css({'background-image': 'url(https://re-publica.de/sites/re-publica.de/files/bgpeople/' + bgimages[Math.floor(Math.random() * bgimages.length)] + ')'});
    }

    var removeBG = function() {
    	$.cookie("nobg_rpTEN", 1, { expires : 30 });
    	$('#bg-switch').removeClass('bg-active').addClass('bg-inactive');
    	$('#block-block-50').css({'background-image': 'none'});
	}

	var addBG = function() {
		setBG();
    	$('#bg-switch').removeClass('bg-inactive').addClass('bg-active');
    	$.removeCookie('nobg_rpTEN', { path: '/' });
	}
	    
    if ($.cookie('nobg_rpTEN')) {
    	$('#bg-switch.bg-active').removeClass('bg-active').addClass('bg-inactive');
    } else {
    	setBG();
    }
 
 	$(document).on('click', '.bg-inactive', function(){ 
 		addBG();
	 });
